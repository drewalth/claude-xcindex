...
// ---------------------------------------------------------------------------
server.tool("xcindex_find_references", "Find every occurrence of a Swift/ObjC symbol in Xcode's pre-built semantic index. " +
    ...
    symbolName: z.string().describe("Exact name of the symbol to look up (e.g. 'UserService', 'fetchUser', 'AuthProtocol'). " +
        "Case-sensitive. Use the declaration name, not a qualified path."),
    projectPath: z.string().optional().describe("Absolute path to the .xcodeproj or .xcworkspace file. " +
        "Used to locate DerivedData automatically. " +
        "Omit if you supply indexStorePath directly."),
    indexStorePath: z.string().optional().describe("Absolute path to the IndexStore DataStore directory " +
    ...
    maxResults: z.number().int().min(1).max(500).optional().default(100).describe("Cap on the number of occurrences returned (default 100, max 500). " +
